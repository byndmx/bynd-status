...
static void bynd_env_get_runtime (void) {
...
static unsigned int bynd_env_get_port (void) {
...
static void bynd_env_get_cerver_receive_buffer_size (void) {
...
static void bynd_env_get_cerver_th_threads (void) {
...
static void bynd_env_get_cerver_connection_queue (void) {
...
static unsigned int bynd_init_env (void) {

	unsigned int errors = 0;

	bynd_env_get_runtime ();

	errors |= bynd_env_get_port ();

	bynd_env_get_cerver_receive_buffer_size ();

	bynd_env_get_cerver_th_threads ();

	bynd_env_get_cerver_connection_queue ();
...
unsigned int bynd_init (void) {

	unsigned int errors = 0;

	if (!bynd_init_env ()) {
		errors |= bynd_service_init ();
	}
