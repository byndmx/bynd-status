...
static void bynd_set_bynd_routes (HttpCerver *http_cerver) {

	/* register top level route */
	// GET /bynd
	HttpRoute *main_route = http_route_create (REQUEST_METHOD_GET, "bynd", bynd_handler);
	http_cerver_route_register (http_cerver, main_route);

	/* register bynd children routes */
	// GET bynd/version
	HttpRoute *version_route = http_route_create (REQUEST_METHOD_GET, "version", bynd_version_handler);
...
static void start (void) {
	...
	if (test_api) {
		...
		bynd_set_bynd_routes (http_cerver);

		// add a catch all route
		http_cerver_set_catch_all_route (http_cerver, bynd_catch_all_handler);

		// admin configuration
		http_cerver_enable_admin_routes (http_cerver, true);
		http_cerver_register_admin_file_system (http_cerver, "/var/uploads");
		http_cerver_register_admin_file_system (http_cerver, "/home/bynd/uploads");
...
int main (int argc, char const **argv) {
	...
	if (!bynd_init ()) {
		...
		cerver_log_error ("Failed to init bynd!");
	}
